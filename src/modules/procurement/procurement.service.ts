...
export class ProcurementService {
  ...
  async updatePO(tenantId: string, id: string, dto: Partial<PurchaseOrder>) {
    await this.findPO(tenantId, id);
    await this.poRepo.update({ id, tenantId }, dto);
    return this.findPO(tenantId, id);
  }

  async removePO(tenantId: string, id: string) {
    await this.findPO(tenantId, id);
  ...
  createMR(tenantId: string, userId: string, dto: Partial<MaterialRequest>) {
    return this.mrRepo.save(this.mrRepo.create({ ...dto, tenantId, createdBy: userId }));
  }

  async approveMR(tenantId: string, id: string, approverId: string) {
    await this.mrRepo.update({ id, tenantId }, { status: MRStatus.APPROVED, approvedBy: approverId });
    return this.mrRepo.findOne({ where: { id, tenantId } });
  }

  async rejectMR(tenantId: string, id: string) {
    await this.mrRepo.update({ id, tenantId }, { status: MRStatus.REJECTED });
    return this.mrRepo.findOne({ where: { id, tenantId } });
  }
  ...
  createInventoryItem(tenantId: string, userId: string, dto: Partial<Inventory>) {
    return this.invRepo.save(this.invRepo.create({ ...dto, tenantId, createdBy: userId }));
  }

  async updateInventoryItem(tenantId: string, id: string, dto: Partial<Inventory>) {
